...
class PolymorphicMetaclass(ModelBase):
    def __new__(cls, name, bases, dct):
        def save(self, *args, **kwargs):
            if hasattr(self, "pre_polymorphic_save"):
                self.pre_polymorphic_save()
                
            if(not self.polymorphic_content_type ):
                self.polymorphic_content_type = ContentType.objects.get_for_model(self.__class__)
            models.Model.save(self, *args, **kwargs)
            
            if hasattr(self, "post_polymorphic_save"):
                self.post_polymorphic_save()
            
            models.Model.save(self)

        def downcast(self):
            model = self.polymorphic_content_type.model_class()
            if (model == self.__class__):
                return self
            return model.objects.get(id=self.id)  

        if issubclass(dct.get('__metaclass__', type), PolymorphicMetaclass):
            dct['polymorphic_content_type'] = models.ForeignKey(ContentType, editable=False, null=True)
            dct['save'] = save
