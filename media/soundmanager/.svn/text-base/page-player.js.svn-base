...
function PagePlayer(oConfigOverride) {
  ...
  sm.url = '/site_media/soundmanager/swf/'; // path to directory containing SM2 SWF
  this.config = {
  ...
  this.handleClick = function(e) {
    ...
    if (thisSound) {
      ...
      var controlParent = document.getElementById('audio-controlpanel');
      thisSound._data = {
        oLink: o, // DOM reference within SM2 object event handlers
        oLI: o.parentNode,
        oControls: self.getElementsByClassName('controls','div', controlParent)[0],
        oStatus: self.getElementsByClassName('statusbar','div',controlParent)[0],
        oLoading: self.getElementsByClassName('loading','div',controlParent)[0],
        oPosition: self.getElementsByClassName('position','div',controlParent)[0],
        oTimingBox: self.getElementsByClassName('timing','div',controlParent)[0],
        oTiming: self.getElementsByClassName('timing','div',controlParent)[0].getElementsByTagName('div')[0],
        oPeak: self.getElementsByClassName('peak','div',controlParent)[0],
        oGraph: self.getElementsByClassName('spectrum-box','div',controlParent)[0],
    ...
    self.lastTrack = o;
    try{
        $('#audioPlayButton')[0].className = "pause";
    }catch(e){
  ...
  this.init = function() {
    ...
    var oLinks = document.getElementsByClassName('playlist-item');
    // grab all links, look for .mp3
    var foundItems = 0;
    for (var i=0; i<oLinks.length; i++) {
      if (oLinks[i].href.indexOf(".mp3")!=-1) {
...
var Metadata = function(oSound) {
  var self = this;
  var oLI = document.getElementById('audioMetadataBlock');
  ...
  this.loadCover = function(link){
      if(link.attributes['coverimage']){
        $('#audioplayercover')[0].src = link.attributes['coverimage'].nodeValue;
      }else{
        $('#audioplayercover')[0].src = "/site_media/soundmanager/cover.jpg";
      }
  }

  this.handleClick2 = function(link) {
    // a sound (or something) was clicked - determine what and handle appropriately
    var o = link;
    var sURL = link.href;
    var soundURL = sURL;
...
function startAudioPlayer(e){
    if(pagePlayer.lastSound){
       var o = pagePlayer.links[pagePlayer.getSoundIndex(pagePlayer.lastTrack)];
    }else{
       var o = pagePlayer.links[0];
    }
    var element = $(e)[0].target;
    if(element){
    ...
    pagePlayer.handleClick2(o);
}


function nextTrack(){
    var index = pagePlayer.getSoundIndex(pagePlayer.lastTrack)+1;
    if(index>=(pagePlayer.links.length)){
       index = 0; 
    }
    pagePlayer.handleClick2(pagePlayer.links[index]);
}
...
function setVolume(volume){
    for(var i=0; i<pagePlayer.links.length; i++){
        soundManager.setVolume("pagePlayerMP3Sound"+i, volume);
    }
}